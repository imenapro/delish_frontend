...
export function ShareInvoiceDialog({ open, onOpenChange, invoice }: ShareInvoiceDialogProps) {
  ...
  const COST_PER_UNIT = 15; // Cost per SMS unit

  const generateMessage = (type: 'brief' | 'detailed') => {
    if (!invoice) return '';

    const date = format(new Date(invoice.created_at), 'dd/MM/yyyy');
    const total = formatCurrency(Number(invoice.total_amount), currency);
...
View: ${invoiceLink}`;
      ...
      const itemsList = invoice.items_snapshot
        ?.map((item: any) => `- ${item.quantity}x ${item.name} (${formatCurrency(Number(item.price * item.quantity), currency)})`)
        .join('\n');
...
Thank you for your business!`;
  ...
  const handleShare = async () => {
    ...
    if (activeTab === 'whatsapp' || activeTab === 'email') {
      try {
        const pdf = generateInvoicePDF(invoice, currency);
                 ...
                 <div className="flex justify-between items-start mb-4">
                   ...
                   <div className="text-right">
                     <div className="flex items-center gap-2 justify-end text-sm font-medium">
                       ...
                       <span>{units} Unit{units > 1 ? 's' : ''} ≈ {formatCurrency(cost, currency)}</span>
                     </div>
