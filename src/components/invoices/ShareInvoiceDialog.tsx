...
export function ShareInvoiceDialog({ open, onOpenChange, invoice }: ShareInvoiceDialogProps) {
  ...
  const generateMessage = useCallback((type: 'brief' | 'detailed') => {
    if (!invoice) return '';
...
View: ${invoiceLink}`;
      ...
      const itemsList = invoice.items_snapshot
        ?.map((item: InvoiceItem) => `- ${item.quantity}x ${item.name} (${Number(item.price * item.quantity).toLocaleString()} RWF)`)
        .join('\n');
...
Thank you for your business!`;
  ...
  }, [invoice, open, smsType, generateMessage]);
