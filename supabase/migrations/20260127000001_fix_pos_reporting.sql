-- Add pos_session_id to orders for better tracking
...
-- Backfill pos_session_id for existing orders based on time overlap
-- This attempts to link orders to the correct session
-- We include a 15-minute buffer before opened_at to handle client/server clock skew
UPDATE public.orders o
SET pos_session_id = ps.id
FROM public.pos_sessions ps
WHERE o.seller_id = ps.user_id
  AND o.shop_id_origin = ps.shop_id
  AND o.created_at >= (ps.opened_at - interval '15 minutes')
  AND (ps.closed_at IS NULL OR o.created_at <= ps.closed_at)
  AND o.pos_session_id IS NULL;

-- Ensure products are viewable by all authenticated users (to fix "Unknown" product names)
...
-- Update process_pos_sale to save pos_session_id
