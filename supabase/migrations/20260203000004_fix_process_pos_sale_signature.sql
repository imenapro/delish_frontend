-- Fix process_pos_sale function signature and logic
-- 1. Drop the incorrect overload (without p_tax_amount) created by previous migrations
-- 2. Update the correct overload (with p_tax_amount) to:
--    a. Include all payment methods in total_sales calculation
--    b. Populate created_by in invoices table (from 20260203000001)
--    c. Correctly handle tax_amount

-- Drop the incorrect overload if it exists
DROP FUNCTION IF EXISTS process_pos_sale(UUID, UUID, UUID, NUMERIC, TEXT, JSONB, TEXT, JSONB);

-- Update the correct overload (with p_tax_amount)
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO invoices (
      ...
      shop_id,
      staff_id,
      created_by, -- Added from 20260203000001
      customer_info,
      items_snapshot,
      subtotal,
      tax_amount,
      ...
      p_items,
      p_total_amount - COALESCE(p_tax_amount, 0), -- Subtotal
      COALESCE(p_tax_amount, 0), -- Tax Amount
      p_total_amount,
      p_payment_method,
      'paid'
    );
