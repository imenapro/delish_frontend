-- Fix ambiguous column reference in get_sales_analytics RPC
...
BEGIN
  ...
  SELECT
    COALESCE(SUM(CASE WHEN o.created_at >= v_today_start THEN o.total_amount ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN o.created_at >= v_week_start THEN o.total_amount ELSE 0 END), 0),
    COALESCE(SUM(CASE WHEN o.created_at >= v_month_start THEN o.total_amount ELSE 0 END), 0)
  INTO
    v_global_daily,
    v_global_weekly,
    v_global_monthly
