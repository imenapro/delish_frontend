...
-- 1. Update has_role to be more robust and check both enum and roles table
CREATE OR REPLACE FUNCTION public.has_role(_user_id UUID, _role_name TEXT)
RETURNS BOOLEAN AS $$
BEGIN
  -- Check user_roles (legacy enum)
  IF EXISTS (
  ...
  -- Check user_role_assignments (dynamic roles)
  IF EXISTS (
    SELECT 1 FROM public.user_role_assignments ura
    JOIN public.roles r ON ura.role_id = r.id
    WHERE ura.user_id = _user_id AND r.name = _role_name
  ) THEN
    RETURN TRUE;
  END IF;
...
-- 2. Update has_permission to bypass checks for super_admin
